...
with open('pizza_count_by_city.csv', mode='w') as new_file:
    write_file = csv.writer(new_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    
    columns = 'city,province,restaurant_count,avg_pizza_price,avg_min_spend'.split(',')
    ...
    for row in f[1:]:
        ...
        if city in cities:
            ...
            entry[1].append(amountMin)
            entry[2][res_id] = priceRangeMin
            cities[city] = entry

        else:
            # province, pizza prices, restaurants
            entry = [province,[amountMin], {}]
            entry[2][res_id] = priceRangeMin
            cities[city] = entry
    ...
    for key in cities:
        item = cities.get(key)
        if mean(item[2].values())< 5: continue
        write_file.writerow([key,item[0],len(item[2].values()), round(mean(item[1]),2), round(mean(item[2].values()),2)])
